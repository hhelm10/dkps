...
General-purpose text encoders trained on sentence-level semantic similarity \citep{reimers2019sbert, su2023instructor} are the standard tool for embedding model outputs, but the notion of similarity they induce is fixed by their training distribution rather than by the use case at hand.
A growing line of work makes the target notion of similarity explicit: conditioning similarity on a stated aspect \citep{deshpande2023csts}, comparing texts through natural-language descriptions of their properties \citep{ravfogel2024description}, and representing a text by an LLM's answers to a set of questions about it \citep{peng2024inbedder, benara2024qaemb}.
\trubric{} is closest to this last family: a trace is represented by an LLM's structured, task-specific summary, and the encoder embeds the summary rather than the raw artifact.
Separately, specialized encoders exist for structured sequential data such as dialogues \citep{liu2022dial2vec} and low-dimensional control trajectories \citep{ge2025trajectory}, but agentic traces -- long, semi-structured tool-use logs -- have primarily been studied through taxonomies of failure modes and actions \citep{cemri2025mast, shu2026traceprobe}, LLM judges of agent behavior \citep{zhuge2025agentjudge, mehtiyev2026beyond}, and trajectory compression for cost reduction \citep{xiao2025agentdiet}.
Consistent with our probing analysis, recent work finds that representations of traces are dominated by the identity of the scaffold and underlying model \citep{oderinwale2026trajectories, sun2025idiosyncrasies}; \trubric{} is, to our knowledge, the first trace representation constructed so that this provenance signal is suppressed while evaluation-relevant content is retained.
