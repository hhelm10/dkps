...
For the target system $f$, we observe traces (and, optionally, trace-level scores) on only a subset $Q \subset Q^{*}$ of $|Q| = m \ll M$ tasks.
Our goal is an estimate $\hat{y}$ of $y(f)$ from the target's $m$ executions together with $\mathcal{D}$.
