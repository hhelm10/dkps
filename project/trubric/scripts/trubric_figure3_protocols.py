        ...
        for m in MS:
            ...
            for cols in draws[m]:
                ...
                for s_ in SIGS:
                    ...
                    # LOO-honest ridge on MDS coords (parity with trubric)
                    n_ = len(D)
                    J = np.eye(n_) - 1 / n_
                    Bmat = -0.5 * J @ (D ** 2) @ J
                    ew, evec = np.linalg.eigh(Bmat)
...
def load_embeddings():
    """(Xc_trubric, Xr_raw, V64) for the q100 panel, both (M, Q, d)."""
    systems, q100, y, B, allowed_llm = load_panel(panel='data/judge/q100.json')
    M, Q = B.shape
    Xc = consensus_center(np.load('data/judge/q100_emb_openai_small.npz')['X'],
                          np.tile(np.arange(Q), M)) \
...
def main_adaptive():
    ...
    for name, allowed in masks.items():
        ...
        for m in MS:
            ...
            store = geom_errs('trubric', cols_of)
...
def main_render(src=OUT_JSON, name='trubric_figure3_protocols'):
    ...
    save_artifact(fig, name, dpi=200, pad=0.02)
