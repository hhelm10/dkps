...
\noindent \textbf{Contribution.} We make four primary contributions.
\begin{enumerate}[itemsep=0pt, topsep=0pt, parsep=2pt]
\item \textbf{Diagnostic:} We show that off-the-shelf semantic embedding functions are not appropriate for efficient evaluation of agentic systems.
Their trace representations recover harness, model family, and exact configuration at well above chance while remaining near chance on outcome, and their prediction error is flat in the number of probe tasks.
...
\qubric{} representations collapse the confounder axes to approximately chance while retaining task and behavioral signal, robustly across seven embedding functions.
...
\item \textbf{Practical:} We release the reference geometry, code, and judged-trace corpus -- constructed from public archives for approximately \$26 -- to support future work on trace-based evaluation.
\end{enumerate}
